...
export default function AlbumDetalle() {
  ...
  const anio = album.fechaLanzamiento ? new Date(album.fechaLanzamiento).getFullYear() : "";
    ...
    <div className="min-h-screen bg-[#0f0b1a] text-white px-10 py-10">
      ...
      <div className="flex items-end gap-6 mb-6">
        <div className="w-32 h-32 md:w-36 md:h-36 rounded-xl bg-gradient-to-br from-[#9a8ef0] to-[#4a3d78] flex items-center justify-center flex-shrink-0 overflow-hidden">
          {album.portada ? (
            <img src={album.portada} alt={`Portada del álbum ${album.nombre}`} className="w-full h-full object-cover" />
          ) : (
        ...
        <div className="flex flex-col gap-1">
          ...
          <span className="text-sm text-[#c7c1de]">
            <span className="font-medium text-white">{album.artista?.alias}</span> · Álbum {anio ? `· ${anio}` : ""}
          </span>
          <span className="text-xs text-[#79738f]">
            {canciones.length} canciones {album.duracion ? `· ${album.duracion}` : ""}
          </span>
        </div>
      </div>
      ...
      <div className="flex flex-col">
        <div className={`grid ${esDueño ? "grid-cols-[24px_1fr_70px_40px]" : "grid-cols-[24px_1fr_70px]"} gap-4 px-3 py-2 text-xs text-[#79738f] border-b border-[#2a2440]`}>
          <span>#</span>
          <span>Título</span>
        ...
        {canciones.length === 0 ? (
          <p className="text-sm text-[#79738f] py-6 text-center">Este álbum aún no tiene canciones</p>
        ) : (
          canciones.map((cancion, index) => (
            <div
              key={cancion.id}
              className={`grid ${esDueño ? "grid-cols-[24px_1fr_70px_40px]" : "grid-cols-[24px_1fr_70px]"} gap-4 px-3 py-2.5 items-center rounded-md hover:bg-white/5 transition-colors group`}
            >
